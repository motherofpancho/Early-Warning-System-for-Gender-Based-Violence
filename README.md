...
This repository contains the development of an Early Warning Model designed to identify the risk of judicial disengagement ("Non-Reporting") in cases of gender-based violence.

The model was built using data provided by the General Directorate of Gender (Municipality of Bahía Blanca) as part of the Machine Learning course for the Specialization in Data Science (UNLaM). This system is a technical research proposal and is not currently in operational use.

The analysis demonstrated how administrative registration failures (such as missing geographic or housing data) act as critical predictors of a victim's detachment from the judicial system. The project's primary value lies in:
•	Invisibility Detection: Identifying that missing data is not "noise" but a signal of structural vulnerability.
•	Sensitivity Prioritization: Optimizing Recall (0.6842) to minimize false negatives within a human protection context.
•	Technical Benchmarking: Validating that a linear model (Ridge Classifier) is more robust and explainable for this specific type of social data than complex boosting models (LightGBM).
